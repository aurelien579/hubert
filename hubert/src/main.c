...
static void user(int key)
{
    ...
    while (1) {
        ...
        case COMMAND_REQUEST:
            ...
            struct command cmd;
            if (!msg_recv_command(user_q, HUBERT_DEST, &cmd)) {
			...
			log_hubert("Sending command to restaurant %d", rest_pid);
            if (!msg_send_command(perm_queue, rest_pid, &cmd)) {
            ...
            if (!msg_send_command_status(user_q, cmd.user_pid, msg.status, msg.time, cmd.name)) {
